...
db.query("CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, body TEXT)");
...
function postQuery(sql) {
  let list = []
  for (const [id, username, body] of sqlcmd(sql)) {
    list.push({ id, username, body })
  }
...
async function list(ctx) {
  let posts = postQuery("SELECT id, username, body FROM posts")
...
async function show(ctx) {
  ...
  let posts = postQuery(`SELECT id, username, body FROM posts WHERE id=${pid}`)
  let post = posts[0]
...
async function create(ctx) {
  ...
  if (body.type === "form") {
    ...
    if (user != null) {
      ...
      sqlcmd("INSERT INTO posts (username, body) VALUES (?, ?)", [user.username, post.body]);
    } else {
...
console.log('Server run at http://127.0.0.1:8027')
await app.listen({ port: 8027 });
